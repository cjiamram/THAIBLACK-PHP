...
function validInput(){
		var flag=true;
		flag=regDec.test($("#obj_weight").val());
		if (flag==false){
			$("#obj_weight").focus();
			return flag;
}
...
function displayData(){
		var url="tbeefgrade/displayData.php?tableName=t_beefgrade&dbName=dbbeefclassify&keyWord="+$("#txtSearch").val();
		$("#tblDisplay").load(url);
}
function createData(){
		var url='tbeefgrade/create.php';
		jsonObj={
			code:$("#obj_code").val(),
			gradeType:$("#obj_gradeType").val(),
...
function readOne(id){
		var url='tbeefgrade/readOne.php?id='+id;
		data=queryData(url);
		if(data!=""){
			$("#obj_code").val(data.code);
			$("#obj_gradeType").val(data.gradeType);
...
function confirmDelete(id){
		...
		if (willDelete.value) {
			url="tbeefgrade/delete.php?id="+id;
